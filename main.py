...
if __name__ == '__main__':
    ...
    model_instance = DeepLearningModel(path=path, feature_range=(0,1))
    cnn_lstm_model_instance = DeepLearningModel(path=path, feature_range=(0,1), time_distributed=True)
    ...
    models = ['one_layer_ff', 'one_layer_cnn', 'one_layer_gru', 'one_layer_lstm', 'multi_layer_ff', 'multi_layer_cnn',
              'multi_layer_gru', 'multi_layer_lstm']

    cnn_lstm_models = ['one_layer_cnn_lstm', "multi_layer_cnn_lstm"]
    ...
    for model in cnn_lstm_models:
        _, metrics, oos_sample_pred = cnn_lstm_model_instance.hypermodel(model)
        results.append(list(metrics.values()))
        oos_sample_preds.append(oos_sample_pred)

    # save metrics and predictionsmode
    metrics_df = pd.DataFrame(results, columns=metrics.keys(), index=models+cnn_lstm_models)
    metrics_df.to_csv(path+'/zero_to_one_MinMaxScaler_metrics.csv', sep=';', decimal='.')
    prediction_df = pd.DataFrame(oos_sample_preds, columns=['Prediction'], index=models+cnn_lstm_models)
